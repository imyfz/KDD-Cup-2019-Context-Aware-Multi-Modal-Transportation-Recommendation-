...
#1.test_queries数据集
queries=read.csv('F:/test/data_set_phase1/test_queries.csv')
nrow(queries)
head(queries)

library(stringr)  #数据分割
...
sum(is.na(nq$pid))/nrow(nq)  #数据的分布的比例,如果不唯一,也就意味着出现了重复的行为
frq=c()
for(i in 1:ncol(nq)){
...
num1[which(num1[,2]==max((num1)[,2])),]  #o中出现次数最多的地方的经纬度

num2=as.data.frame(table(queries$d))
tail(num2[order(num2$Freq),])
num2[which(num2[,2]==max((num2)[,2])),]  #d中出现次数最多的地方的经纬度
...
#2.test_plans数据集
tplans=read.csv('F:/test/data_set_phase1/test_plans.csv');str(tplans)
nm=nrow(tplans);nm
...
head(cdf)
cdf=cdf%>%arrange(id)
sum(is.na(cdf$price))  #仅有单个plan的样本没有中位数可以插值，可以将缺失值设置为0
...
dim(nq);colnames(nq) #queries
dim(tp);colnames(tp)  #train_plans
...
colnames(dat)
library(lubridate)
time=dat$req_time   #时间特征
req_week=wday(time);head(req_week)
req_hour=hour(time);head(req_hour)
testdata=cbind(dat,req_week)
testdata=cbind(testdata,req_hour)
head(testdata)
nrow(testdata)
testdata=testdata[,-3]
colnames(testdata)

write.csv(testdata,'F:/test/KDD/testdata.csv')
